...
class RAGPipeline:
    ...
    def ingest_documents(self, 
        ...
        print(f"  - Ready for search and Q&A")
        print(f"{'='*70}\n")
...
def demo():
    ...
    documents = [
        ...
        and control over their search pipeline. Unlike black-box solutions like LangChain or 
        ElasticSearch, VectorFlow exposes every internal layer: embedding, indexing, ranking, 
        and generation. The system is built to be reproducible, swappable, and benchmarkable.""",
        ...
        """Hybrid search is the core retrieval strategy in VectorFlow-RAG. It combines 
        BM25 lexical matching with vector similarity search. BM25 is excellent at exact 
        keyword matches and handles rare terms well, while vector search captures semantic 
        similarity and understands context. The fusion parameter alpha controls the balance 
        between the two approaches, where alpha=0.5 gives equal weight to both methods.""",
        
        """For text generation, VectorFlow-RAG uses Ollama to run large language models locally.
        This approach ensures complete privacy and eliminates API costs. Popular models include 
        TinyLlama (600MB), Llama 3.2 (1.3GB), and Mistral (4GB), which can all run on consumer 
        hardware. The system supports any Ollama model through simple configuration changes.
        ...
        """VectorFlow-RAG is built with modern ML engineering practices. It uses FastAPI for 
        REST API endpoints, Docker for containerization, and GitHub Actions for CI/CD.
        The system can be deployed to free platforms like Streamlit Community Cloud, Render,
        or Hugging Face Spaces. All components are modular and can be replaced independently.
